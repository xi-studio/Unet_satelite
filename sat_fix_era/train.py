...
def training_function(args, config):
    ...
    train_files = np.arange(100) 
    valid_files = np.arange(100)


    if args.fake == False:
        train_files = np.load(args.filenames + '/train.npy')
        valid_files = np.load(args.filenames + '/valid.npy')

        
    train_ds = Radars(train_files, fake) 
    valid_ds = Radars(valid_files, fake)
